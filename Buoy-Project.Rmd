...
##Methodology
How we combine different data set and how we clean it using certain methodolody

For our dataset, we choose from year 1999-2019, which are 20 year. When we first look at 20 years data of the Boston buoy, we noticed that different years have different column names. For example, for the year of 1999 and 2000, the "Tide" column has no value. Then from 2005-2019, each dataset has one more column called "mm" which means the mintues.  
...
```
```{r}
plot(Buoy2$PRES, Buoy2$ATMP, xlim = c(960, 1050), xlab = "Sea Level Pressure", ylab = "Air Temperature")
```
